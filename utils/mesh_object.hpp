...
public:
	...
	void addMaterial(std::string aMode, MaterialParameters aMaterialParams) { // material params are the uniforms in the shader
	...
	std::optional<RenderData> getRenderData(const RenderOptions &aOptions) const override {
		...
		return std::optional<RenderData>(RenderData{
				getModelMatrix(), // where in the scene and how oriented
